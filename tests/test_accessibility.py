...
class TestAccessibilityRoutes:
    ...
    @pytest.mark.asyncio
    async def test_get_take_over_all_stands_all_busy(self, api_client) -> None:
        params = {"platform": "ios"}
        res = await api_client.get(url="/take_over_all_stands", params=params)
        await api_client.aclose()
        assert res.status_code == 200
    ...
    @pytest.mark.dependency(depends=["all_busy"])
    @pytest.mark.asyncio
    async def test_get_available_stand_all_busy(self, api_client) -> None:
        params = {"platform": "ios"}
        res = await api_client.get(url="/get_available_stand", params=params)
        await api_client.aclose()
        assert res.status_code == 200
    ...
    @pytest.mark.asyncio
    async def test_get_vacate_all_stands_all_available(self, api_client) -> None:
        params = {"platform": "ios"}
        res = await api_client.get(url="/vacate_all_stands", params=params)
        await api_client.aclose()
        assert res.status_code == 200
